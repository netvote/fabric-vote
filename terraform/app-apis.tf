...
resource "aws_api_gateway_resource" "admin_ballot_id_share" {
  rest_api_id = "${aws_api_gateway_rest_api.netvote_mobile_api.id}"
  parent_id = "${aws_api_gateway_resource.admin_ballot_id.id}"
  path_part = "share"
}

resource "aws_api_gateway_method" "admin_share_ballot" {
  rest_api_id = "${aws_api_gateway_rest_api.netvote_mobile_api.id}"
...
resource "aws_api_gateway_integration" "admin_share_ballot" {
  ...
  integration_http_method = "POST"
  type = "AWS_PROXY"
  uri = "arn:aws:apigateway:${var.region}:lambda:path/2015-03-31/functions/${aws_lambda_function.admin_share_ballot.arn}/invocations"
}
