...
exports.handler = (event, context, callback) => {
    ...
    nJwt.verify(event.authorizationToken,signingKey,function(err,verifiedJwt){
        if(err){
            console.log(err);
            ...
            var principalId = verifiedJwt.body.accountId+"|"+verifiedJwt.body.voteId;
            const policy = new AuthPolicy(principalId, awsAccountId, apiOptions);
            policy.allowAllMethods();
            var result = policy.build();
            result.context = {
                "widgetId": verifiedJwt.body.widgetId,
                "voterId": verifiedJwt.body.voteId,
                "accountId": verifiedJwt.body.accountId
            };
